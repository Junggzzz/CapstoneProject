    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 md:top-6 md:right-6 text-white/70 hover:text-white bg-black/50 p-2 rounded-full transition-colors z-50"
            aria-label="Tutup"
          >
          ...
          <button 
            onClick={onPrev}
            className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 text-white/70 hover:text-white bg-black/50 p-2 md:p-3 rounded-full transition-colors z-50"
          ...
          <button 
            onClick={onNext}
            className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 text-white/70 hover:text-white bg-black/50 p-2 md:p-3 rounded-full transition-colors z-50"
          ...
          {/* Image Container */}
          <div className="relative w-full h-full max-w-5xl max-h-[65vh] md:max-h-[80vh] mx-auto px-4 md:px-16 flex items-center justify-center">
            <motion.div
              key={currentIndex}
            ...
            >
              <Image
                ...
                quality={90}
              />
              <div className="absolute bottom-[-100px] md:bottom-[-70px] left-0 right-0 flex flex-col items-center justify-center text-white/80 gap-2 px-4 text-center">
                <p className="text-sm md:text-lg font-medium line-clamp-1">{currentImage.title}</p>
                <div className="flex items-center gap-3 md:gap-4">
                  <p className="text-xs md:text-sm opacity-60">{currentIndex + 1} / {images.length}</p>
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 bg-[#25D366] hover:bg-[#20ba5a] text-white text-[11px] md:text-xs font-semibold px-3 py-1.5 md:px-4 md:py-2 rounded-full transition-colors shadow-lg"
                  >
